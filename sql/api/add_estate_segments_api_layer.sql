-- Add the first Silver-powered public API table for sale-market segments.
--
-- This table exposes only aggregated room/area sale metrics. It does not
-- expose listing-level data, URLs, descriptions, authors, or source IDs.

begin;

create table if not exists public.api_estate_segments_current (
    date date not null,
...
create index if not exists api_estate_segments_city_sector_idx
    on public.api_estate_segments_current (city, sector);

create index if not exists api_estate_segments_rooms_area_idx
    on public.api_estate_segments_current (rooms_group, area_band);

alter table public.api_estate_segments_current enable row level security;

drop policy if exists "Public can read estate segment API data"
    on public.api_estate_segments_current;
create policy "Public can read estate segment API data"
    on public.api_estate_segments_current
    for select
    to anon, authenticated
    using (true);

grant select on public.api_estate_segments_current to anon, authenticated;
grant select, insert, update, delete
    on public.api_estate_segments_current to service_role;

revoke insert, update, delete, truncate, references, trigger
    on public.api_estate_segments_current from anon, authenticated;

comment on table public.api_estate_segments_current is
    'Public API table with aggregated sale metrics by rooms and area band.';
